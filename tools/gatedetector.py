...
class GateDetector:
    ...
    def start(self):
        self.zconf = Zeroconf()
        self.zconf.add_service_listener("_gatenode._tcp.local.", self)
    ...
    def choose_gate(self):
        ...
        while ret == None:
            ...
            nbmax = len(lst)
            if nbmax == 1:
                yn = yn_input('No gate found, search again?')
                if yn == 'N': break
            else:
                nb = choice_input(lst, 'Which gate?')
                if nb and nb > 1:
                    name = lst[nb - 1]
...
if __name__ == '__main__':
    ...
    def action__list():
        while True:
            gates = detector.detected_gates()
            for k in gates: print(gates[k])
            if yn_input('List again?', 'Y') == 'N': break

    def action__choose():
        print('You chose:', detector.choose_gate())
